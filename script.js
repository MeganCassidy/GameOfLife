...
//Allowing user to amend colors for dead and alive cells 
let colorSelector;
...
function update(event) {
    ...
    if (clickbtn) {
        clickbtn.style.backgroundColor = event.target.value;
      };

  }

  function update(event) {
    let alive = document.querySelector("#worldgrid");
    let body = document.querySelector("body");
    let clickbtn = document.querySelectorAll("button");

    alive.style.backgroundColor = event.target.value;
  ...
  // Basic dark mode function - changing body of page 
function darkMode() {
...
let evolutionSpeed=600;// 0.6 second between generations
...
function startStopGol(){
    ...
    if (!started) {
       started = true;
       startstop.value='Pause Game';
       evolve();
     
     } else {
        started = false;
        startstop.value='Start Game';
        clearTimeout(timer); 
    }
}
